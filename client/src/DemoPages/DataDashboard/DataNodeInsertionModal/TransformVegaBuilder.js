...
const filterOptions = [
  {
    label: 'Comparison',
    options: ComparisonExpressions.map(operand => ({
      label: operand,
      value: operand,
    })),
  },
  {
    label: 'Type checking',
    options: TypeCheckingFunctions.map(operand => ({
      label: operand,
      value: operand,
    })),
  },
...
class TransformVegaBuilder extends React.Component {
  ...
  buildVega(newChange) {
    if (this.state.type === 'filter') {
      this.setState(
        { vegaFilter: { ...this.state.vegaFilter, ...newChange } },
        () => {
      ...
      <Fragment>
        ...
        <Row className="form-group">
              ...
              <Row className="form-group">
                ...
                <Col>
                  <Creatable
                    ...
                    onChange={newField =>
                      this.buildVega({ field: newField.value })
                    }
                  />
                </Col>
              </Row>
              ...
              <Row className="form-group">
                ...
                <Col>
                  <Select
                    ...
                    onChange={newOperand =>
                      this.buildVega({ operand: newOperand.value })
                    }
                  />
                </Col>
              </Row>

              {ComparisonExpressions.includes(
                this.state.vegaFilter.operand,
              ) && (
                <Row className="form-group">
                  ...
                  <Col>
                    <Input
                      type="text"
                      placeholder="Enter the compared value here"
                      onChange={e =>
                        this.buildVega({ threshold: e.target.value })
                      }
                    />
                  </Col>
                </Row>
          ...
          {this.state.type === 'aggregate' && (
            <Col>
              <Row className="form-group">
                <Label for="aggregation" md={4}>
                  Functions
                </Label>
                <Col>
                  <Creatable
                    isMulti
                    options={AggregationMethods.map(method => ({
                      value: method,
                      label: method,
                    }))}
                    ...
                    onChange={newMethods =>
                      this.buildVega({
                        ops: newMethods.map(item => item.value),
                      })
                    }
                  />
                </Col>
              ...
              <Row className="form-group">
                ...
                <Col>
                  <Creatable
                    ...
                    onChange={newFields =>
                      this.buildVega({
                        fields: newFields.map(item => item.value),
                      })
                    }
                  />
                </Col>
            ...
            <JSONView
              src={
                this.state.type === 'aggregate'
                  ...
                  : {
                      type: this.state.vegaFilter.type,
                      expr: this.state.vegaFilter.expr,
                    }
              }
            />
          </Col>
